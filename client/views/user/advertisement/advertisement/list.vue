...
<script>
  ...
  export default {
    ...
    computed: {
      editApi () {
        return `${this.API.url}/advertisement/advertisement/save${this.API.suf}`
      },
      delApi () {
        return `${this.API.url}/advertisement/advertisement/delete${this.API.suf}`
      },
      customApi () {
        return `${this.API.url}/advertisement/ad_customer/list${this.API.suf}`
      },
      dicDetailApi () {
    ...
    methods: {
      ...
      getItems (page, pageSize, searchOpt = {}) {
        ...
        self.axios({
          url: this.API.HOST + this.API.advertisement.path + this.API.suf,
          method: this.API.advertisement.type,
          params: listObj
        }).then((res) => {
          console.log('广告管理')
          console.log(res)
