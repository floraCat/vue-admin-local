...
<script>
  ...
  export default {
    ...
    computed: {
      editApi () {
        return `${this.API.url}/advertisement/ad_resource/update${this.API.suf}`
      },
      delApi () {
        return `${this.API.url}/advertisement/ad_resource/delete${this.API.suf}`
      }
    },
    ...
    methods: {
      ...
      getItems (page, pageSize, searchOpt = {}) {
        ...
        self.axios({
          url: this.API.HOST + this.API.ad_resource.path + this.API.suf,
          method: this.API.ad_resource.type,
          params: listObj
        }).then((res) => {
          console.log('广告资源管理')
          console.log(res)
