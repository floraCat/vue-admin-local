...
<script>
  ...
  export default {
    ...
    computed: {
      listApi () {
        return `${this.API.url}/delivery/list${this.API.suf}`
      },
      editApi () {
        return `${this.API.url}/delivery/save${this.API.suf}`
      },
      delApi () {
        return `${this.API.url}/delivery/delete${this.API.suf}`
      },
      fieldItemsApi () {
        return `${this.API.url}/deliveryarea/list${this.API.suf}`
      }
    },
      ...
      getItems (page, pageSize, searchOpt = {}) {
        ...
        self.axios({
          ...
          if (res.data.status === 'failure') {
            console.log('配送员数据错误提示：' + res.data.message);
          }
