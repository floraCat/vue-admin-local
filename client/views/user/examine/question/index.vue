<template>
	<form-crud 
	field="question" 
	spacComp = "viewQuestion"
	:items="items" :ables="ables" :apiObj="{
                edit: editApi,
                del : delApi,
                }" :noObj="{
                add : false,
...
<script>
	...
	export default {
		...
		computed: {
			listApi() {
				return `${this.API.url}/topic/list${this.API.suf}`
			},
			editApi() {
				return `${this.API.url}/topic/save${this.API.suf}`
			},
			delApi() {
				return `${this.API.url}/topic/delete${this.API.suf}`
			}
		},
				...
				self.axios({
					...
					if(res.data.status === 'success') {
						...
						for(let i = 0; i < list.length; i++) {
							...
							if(list[i].topicStatus) {
								list[i].topicStatus = parseInt(list[i].topicStatus)
							}
						}
						/*-----格式化数据 end------*/
						self.items = list
